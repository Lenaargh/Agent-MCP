...
from ..core.config import logger, get_db_path, get_rag_db_path
...
from .portable import is_postgres_configured, get_postgres_connection
...
def get_db_connection():
    """
    Returns a connection to the core relational database (agents, tasks,
    messages, oauth bindings, project context, ...).

    PostgreSQL is used when DATABASE_URL is set; otherwise this falls back
    to the local SQLite file, which is the default for local development.
    The returned object exposes the same surface either way: `.cursor()`,
    `.commit()`, `.rollback()`, `.close()`, and `row["column"]` access.
    """
...
def get_rag_db_connection() -> sqlite3.Connection:
    """
    Returns a connection to the local SQLite file used for RAG chunks, RAG
    metadata, and vector embeddings. Always SQLite regardless of
    DATABASE_URL: this content is fully regenerated by the RAG indexer from
    source material, so it intentionally stays on local disk rather than
    requiring durable cloud storage. When the app is running entirely on
    SQLite (no DATABASE_URL), this points at the same single database file
    as get_db_connection(), matching historical behavior.
    """
    return _get_sqlite_connection(get_rag_db_path())


def _get_sqlite_connection(db_file_path: Path) -> sqlite3.Connection:
    """
