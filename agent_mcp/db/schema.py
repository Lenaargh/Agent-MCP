...
from .connection import (
    get_db_connection,
    get_rag_db_connection,
    check_vss_loadability,
    is_vss_loadable,
)
from .portable import is_postgres_configured
...
def _init_core_sqlite_tables(conn: sqlite3.Connection) -> None:
    """Creates the core relational tables (agents, tasks, messages, ...) in
    the given SQLite connection. Used only for the local-development,
    single-file-SQLite deployment path; when PostgreSQL is configured these
    tables are managed by Alembic instead (see pg_bootstrap.py)."""
    cursor = conn.cursor()
...
def _init_rag_sqlite_tables(conn: sqlite3.Connection) -> None:
    """Creates the RAG tables (rag_chunks, rag_meta, rag_embeddings) in the
    given SQLite connection. These always live on local SQLite regardless of
    DATABASE_URL, since RAG content is fully regenerable from source."""
    cursor = conn.cursor()
...
def init_database() -> None:
    """
    Initializes the database schema. Never silently drops or recreates
    tables that already exist.

    - PostgreSQL configured (DATABASE_URL set): the core relational tables
      are managed by versioned Alembic migrations (`alembic upgrade head`,
      applied automatically). The RAG tables still get their local SQLite
      file initialized (unchanged CREATE TABLE IF NOT EXISTS logic), since
      RAG content intentionally stays local.
    - No DATABASE_URL (local development default): unchanged historical
      behavior — every table lives in one local SQLite file.
    """
    logger.info("Initializing database schema...")

    if is_postgres_configured():
        from .pg_bootstrap import run_postgres_migrations

        run_postgres_migrations()

        rag_conn = get_rag_db_connection()
        try:
            _init_rag_sqlite_tables(rag_conn)
            rag_conn.commit()
            logger.info(
                "Database schema initialized successfully (PostgreSQL core + local RAG cache)."
            )
        except sqlite3.Error as e:
            logger.error(
                f"A database error occurred initializing the local RAG cache: {e}",
                exc_info=True,
            )
            rag_conn.rollback()
            raise RuntimeError(f"Failed to initialize RAG cache schema: {e}") from e
        finally:
            rag_conn.close()
        return
    ...
    try:
        ...
        _init_core_sqlite_tables(conn)
        _init_rag_sqlite_tables(conn)
        conn.commit()
